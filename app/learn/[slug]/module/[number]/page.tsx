                ...
                <div className="prose max-w-none mb-8 overflow-hidden">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      pre: ({ children }) => (
                        <pre className="bg-gray-50 border border-gray-200 rounded-lg my-4 overflow-x-auto max-w-full">{children}</pre>
                      ),
                      code: ({ children, className }) => {
                        const isInline = !className
                        return isInline ? (
                          <code className="bg-orange-50 text-[#FF6F00] px-1 py-0.5 rounded text-sm font-mono break-all">{children}</code>
                        ) : (
                          <code className="block text-gray-800 p-4 text-sm font-mono whitespace-pre-wrap break-words">{children}</code>
                        )
                      },
                      blockquote: ({ children }) => (
                        <blockquote className="border-l-4 border-[#FF6F00] pl-4 italic text-gray-600 my-4">{children}</blockquote>
                      ),
                      table: ({ children }) => (
