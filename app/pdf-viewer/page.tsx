...
// Renders a PDF using PDF.js (CDN) — clean canvas output, no browser chrome
...
export default async function PdfViewerPage({ searchParams }: Props) {
  ...
  if (!url) {
    return <div style={{ padding: 40, fontFamily: 'sans-serif', color: '#666' }}>No PDF URL provided.</div>
  }
...
<script>
  ...
  pdfjsLib.getDocument(PDF_URL).promise.then(pdf => {
    ...
    if (totalPages <= 10) {
      // Render all pages at once for short PDFs
